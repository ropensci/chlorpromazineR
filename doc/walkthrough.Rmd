...
We hope this package has several benefits:

* improve transparency and consistency in calculating CPZ equivalents
* reduce human error and improve accuracy
* simplify workflows for large datasets, as from chart reviews of electronic health records

We use the data from the International Consensus Study (Gardner et al. 2010) as the default conversion "key" and include the DDD conversion factors as reported by Leucht et al. 2016. The user can also create their own keys (and contribute them to this package), which are `list` objects as described below.

The default key is `gardner2010`, derived from the 2010 paper by Gardner et al. In this paper, no equivalency was provided for short acting vs. oral chlorpromazine, so the default `gardner2010` key only includes oral and long acting injectable conversion factors. However, the SAI equivalencies are available in terms of chlorpromazine SAI equivalents, so to use that data, the key `gardner2010_withsai` can be used if it is understood that the `sai` and `oral` are doses are not directly comparable (but could be converted using an external reference value).
